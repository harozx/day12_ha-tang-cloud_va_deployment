...
### Health Check
```bash
curl -i https://day12-production-agent.up.railway.app/health
# Expected: {"status": "ok", "version": "1.0.0", "environment": "production", ...}
```

### Readiness Check
```bash
curl -i https://day12-production-agent.up.railway.app/ready
# Expected: {"ready": true}
```
...
```bash
for i in {1..15}; do 
  curl -H "X-API-Key: test-api-key" https://day12-production-agent.up.railway.app/ask \
